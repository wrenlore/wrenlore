...
export class SearchService {
  ...
  async searchPage(
    ...
    if (opts.userId) {
      queryResults = queryResults.where(
        this.accessiblePagePredicate(opts.userId),
      );
    }
    ...
    // Defensive permission filter; authenticated search also excludes restricted
    // pages in SQL before ranking/limit so inaccessible results cannot crowd out
    // accessible ones.
  ...
  private accessiblePagePredicate(userId: string) {
    return sql<SqlBool>`
      NOT EXISTS (
        WITH RECURSIVE ancestors AS (
          SELECT p.id AS ancestor_id, p.parent_page_id
          FROM pages p
          WHERE p.id = pages.id
          UNION ALL
          SELECT parent.id AS ancestor_id, parent.parent_page_id
          FROM pages parent
          JOIN ancestors a ON a.parent_page_id = parent.id
        )
        SELECT 1
        FROM ancestors a
