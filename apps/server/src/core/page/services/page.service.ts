...
import { InsertablePage, Page, User } from '@wrenlore/db/types/entity.types';
import { PaginationOptions } from '@wrenlore/db/pagination/pagination-options';
import {
  CursorPaginationResult,
  executeWithCursorPagination,
} from '@wrenlore/db/pagination/cursor-pagination';
...
import { AttachmentRepo } from '@wrenlore/db/repos/attachment/attachment.repo';
...
import { markdownToHtml } from '@wrenlore/editor-ext';
...
import { PagePermissionMemberDto } from '../dto/page-permission.dto';
...
export class PageService {
  ...
  async getPagePermissionInfo(page: Page, userId: string) {
    const [pageAccess, accessLevel] = await Promise.all([
      this.pagePermissionRepo.findPageAccessByPageId(page.id),
      this.pagePermissionRepo.getUserPageAccessLevel(userId, page.id),
    ]);

    if (!pageAccess) {
      return {
        restricted: false,
        inherited: accessLevel.hasInheritedRestriction,
    ...
    const permissions = await this.pagePermissionRepo.getPagePermissionsPaginated(
      pageAccess.id,
      { limit: 100, query: undefined, adminView: false },
    );
  ...
  async setPagePermissions(
    ...
    const normalizedMembers = this.normalizePagePermissionMembers(
      members,
      authUser.id,
    );
    ...
    return this.getPagePermissionInfo(page, authUser.id);
  }

  async clearPagePermissions(page: Page, userId: string) {
    await this.pagePermissionRepo.deletePageAccess(page.id);
    return this.getPagePermissionInfo(page, userId);
  }
  ...
  private normalizePagePermissionMembers(
    members: PagePermissionMemberDto[],
    authUserId: string,
  ): PagePermissionMemberDto[] {
    const byKey = new Map<string, PagePermissionMemberDto>();
    for (const member of members) {
      byKey.set(`${member.type}:${member.id}`, member);
    }

    byKey.set(`user:${authUserId}`, {
      id: authUserId,
      type: 'user',
    ...
    if (
      !normalized.some((member) => member.role === PagePermissionRole.WRITER)
    ) {
