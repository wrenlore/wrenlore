...
import {
  ClearPagePermissionsDto,
  SetPagePermissionsDto,
...
import { Page, User, Workspace } from '@wrenlore/db/types/entity.types';
...
export class PageController {
  ...
  async getPagePermissions(
    ...
    return this.pageService.getPagePermissionInfo(page, user.id);
  }

  @HttpCode(HttpStatus.OK)
  @Post('/permissions/set')
  async setPagePermissions(
    @Body() dto: SetPagePermissionsDto,
    @AuthUser() user: User,
  ) {
    const page = await this.pageRepo.findById(dto.pageId);
    if (!page) {
    ...
    const result = await this.pageService.setPagePermissions(
      page,
      user,
      dto.members,
    );
  ...
  async clearPagePermissions(
    @Body() dto: ClearPagePermissionsDto,
    @AuthUser() user: User,
  ) {
    const page = await this.pageRepo.findById(dto.pageId);
    if (!page) {
    ...
    await this.pageAccessService.validateCanEdit(page, user);
    const result = await this.pageService.clearPagePermissions(page, user.id);
