...
function isGenerativeAiEnabled() {
  return getDefaultStore().get(workspaceAtom)?.settings?.ai?.generative === true;
}
...
const CommandGroups: SlashMenuGroupedItemsType = {
  basic: [
    {
      title: "Ask AI",
      description: "Generate or transform text at the cursor.",
      searchTerms: ["ai", "generate", "rewrite", "summarize"],
      icon: IconSparkles,
      command: ({ editor, range }: CommandProps) => {
        editor.chain().focus().deleteRange(range).run();
        editor.commands.blur();
        requestAnimationFrame(() => {
          getDefaultStore().set(showAiMenuAtom, true);
        });
      },
    },
    {
...
export const getSuggestionItems = ({
  ...
  for (const [group, items] of Object.entries(CommandGroups)) {
    const filteredItems = items.filter((item) => {
      if (item.title === "Ask AI" && !isGenerativeAiEnabled()) {
        return false;
      }
