...
import { useAiGenerateStreamMutation } from "@/wrenlore/ai/queries/ai-query.ts";
...
const EditorAiMenu = ({ editor }: EditorAiMenuProps): JSX.Element | null => {
  ...
  const handleGenerate = useCallback(
    (item?: CommandItem) => {
      ...
      generateAiContent({
        ...
        content,
        onChunk: (chunk) => {
          setOutput((currentOutput) => currentOutput + chunk.content);
        },
        onComplete: () => {
          setPrompt("");
          setIsLoading(false);
        ...
        onError: (error) => {
          ...
          notifications.show({
            ...
            message:
              error.error ||
              "Ask AI is unavailable. Check the AI provider configuration.",
          });
        },
      });
      setLastAction(command);
    },
    [editor, prompt, isLoading, generateAiContent, resetMenu],
  );
  ...
  const handleCommand = useCallback(
    ...
    [editor, output, lastAction, handleGenerate, resetMenu, setShowAiMenu],
  );
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (event.key === "ArrowDown" || event.key === "ArrowUp") {
        ...
        return setSelectedIndex((currentSelectedIndex) => {
    ...
    [currentItems, selectedIndex, handleCommand, setShowAiMenu],
  );
  ...
  }, [editor, updateMenuPlacement, debouncedUpdateMenuPlacement, setShowAiMenu]);

  useEffect(() => {
    setShowAiMenu(false);
  }, [location, setShowAiMenu]);
